...
function createChart(event) {
    ...
    $.ajax({
        url: url,
        type: $(urlFormId).attr("method"),
        success: function (result) {
            var top5List = JSON.parse(result);
            if (top5List.length === 0) {
                Materialize.toast($("#resultMissed").html());
                return;
            }
            ...
            if (!builded) {
                buildChart(top5List, chartContainer);
                builded = true;
            }
        ...
        error: function (error) {
            console.log("Can't get top5 list: " + error);
            Materialize.toast("Server error, try again!", 4000);
        }
    });
}

// Build chart in container from data
function buildChart(list, container) {
    var seriaData = [];
    $.each(list, function (index, value) {
        var name = value["user"]["form"]["lastName"] + " "
...
$(function () {
    ...
    $("#create-chart").click(eventParamsForTop5Chart, createChart);
});